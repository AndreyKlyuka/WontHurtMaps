...
| ID     | Requirement                                                                                 | Priority   |
| ------ | ------------------------------------------------------------------------------------------- | ---------- |
| FR-2.1 | Preprocessing: emoji removal, Unicode normalization                                         | Must       |
...
| ID     | Requirement                                                               | Priority |
| ------ | ------------------------------------------------------------------------- | -------- |
| FR-3.1 | Fallback chain: cache → Google Maps Geocoding API → district dictionary   | Must     |
...
| FR-3.5 | Geocoding queue overflow protection (max 500 per cycle)                   | Must     |
...
| NFR-2.4 | External service degradation | Graceful fallback (Google Maps Geocoding API, OSRM)          |
...
- `slang_dictionary` — admin reference table for local terms (not used in pipeline)
...
| Google Maps Geocoding API | Primary geocoder (MVP) | Paid API, $200/month free credit (~40k req), high availability | District dictionary + geocode cache |
...
- Local Photon geocoder (add if Google Maps billing becomes a concern)
...
| Low extraction accuracy    | Map data unreliable                 | Medium      | LLM with dynamic few-shot pool, admin review queue, admin_verified feedback loop  |
| Google Maps data staleness | Wrong geocoding for renamed streets | Low         | `street_renames` table with admin validation                                      |
| OSRM downtime              | Route check unavailable             | Low         | Graceful degradation (503), feature non-critical                                  |
| Google Maps billing        | Unexpected API costs                | Low         | 90-day geocode cache limits requests; monitor usage dashboard                     |
